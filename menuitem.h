    ...
    menu_ptr->submenu = {
        ...
        {"network +", {{"ip +", {{"ip addr +", {{"ip addr show +", {{"show netdev info including ip address#ip addr show ${dev}#"},
                       ...
                       {"netstat +", {{"show ip routing tables (ip:numerical)#netstat -rn#"},
                                      ...
                                      {"show statistics of pkts of each interface#netstat -i#"}}},
        ...
        {"docker +", {{"login +", {{"*login user/pwd to dockerhub#docker login#dockerlogin"},
                      ...
                      {"container +", {{"show all containers dead or alive (detailed)#docker ps -a#"},
                                       {"show all containers dead or alive (only id)#docker ps -a -q#"},
                                       {"show all containers belong to certain image#docker ps -a --filter ancestor=${image:tag}#"},
                                       ...
                                       {"show the filesystem diff between container w.r.t its image (A: add, C: change)#docker container diff ${containerid}#"},
                                       ...
                                       {"attach to an existed running container#docker attach ${container_name/id}#"},
                      ...
                      {"files +"}}},
        {"filesystem & devices +", {{"file permissions +", {{"change owner:group +", {{"change owner:group recursively for dir/subdir#sudo chown -R ${owner}:${group} ${dir}#"}}},
                                                           {"change rwx +"}}},
                                   {"file time +", {{"set certain file atime/mtime as date:time#touch -d ${date:-2023-11-22} ${time:-10:27:46} ${file}#"}}},
                                   ...
                                   {"tar +", {{"extract the content of xxx (tar)#tar -xvf ${xxx.tar}#"},
                                              ...
                                              {"zip files under dir using gzip (tar.gz)#tar -czvf ${xxx.tar.gz} ${dir}#"},
        ...
        {"search +", {{"find +", {{"show manual of find#man find#"},
                      ...
                      {"grep +", {{"search for a match to pattern in certain file#grep '${pattern}' ${file}#"},
                                  ...
                                  {"pattern +", {{"search for plain text without regex in file/dir#grep -e ${no_regex_pattern} ...#"},
                                                 {"search for plain text without regex (only output the matched part)#grep -oe ${no_regex_pattern} ...#"},
        ...
        {"alpine +", {{"apk official doc site#https://wiki.alpinelinux.org/wiki/Main_Page#"},
                      ...
                      {"openrc +", {{"install openrc for alpine#apk add openrc#"},
                                    {"#rc-service#"}}}}},
