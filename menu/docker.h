    ...
    docker_ptr->submenu = {
        {"docker +", {
            ...
            {"image +", {{"show all images#docker images#"},
                         ...
                         {"save docker image as tarball (history & metadata preserved)#docker save -o ${output}.tar ${image}:${tag}#"},
                         {"load tarball into docker image (history & metadata restored)#docker load -i ${input.tar}#"},
                         {"export docker image as tarball (just a snapshot)#docker export -o ${out.tar} ${container_id}#"},
                         {"import docker image as tarball (snapshot recovery)#docker import ${in.tar}#"}}},
            ...
            {"container +", {{"show containers alive#docker ps#"},
                             {"show containers dead or alive (detailed)#docker ps -a#"},
                             {"show containers dead or alive (only id)#docker ps -a -q#"},
                             {"show containers with exited status#docker ps -a --filter 'status=exited'#"},
                             ...
                             {"lifetime manage +", {{"remove all stopped container#docker container prune#"},
                                                    {"stop container processes#docker stop ${container id list}#"},
                                                    {"issue signal to container process#docker kill -s ${sigid} ${container id list}#"},
                                                    {"rm container instance forcibly#docker rm -f ${container list}#"}}}}},
