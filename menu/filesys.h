...
MenuItem* Method::get_filesys_ptr(){
    ...
    filesys_ptr->submenu = {
        {"file & filesystem +", {
            {"file +", {{"file permissions +", {{"change owner:group +", {{"change owner:group recursively for dir/subdir#sudo chown -R ${owner}:${group} ${dir}#"}}},
                                                {"change rwx +"}}},
                        ...
                        {"file cleanup +", {{"truncate to 0 len by redirection#> ${filename}#"},
                                            {"truncate to 0 len by cmd#truncate -s 0 ${filename}#"},
                                            {"truncate to 0 len by null operator#: > ${filename}#"},
                                            {"cat /dev/null > ${filename}"}}}}},
            {"dir +", {{"create dir under $PWD#mkdir ${dirname}#"},
                       ...
                       {"is dir empty or not (oneline shell)#[ $(ls -A ${dir} | wc -l) -eq 0 ] && echo 'empty' || echo 'not empty'#"},
            ...
            {"base64 +", {{"show help#base64 --help#"},
                          {"encode normal file to base64 file#base64 ${in_file} | cat > ${out_file.base64}#"},
                          {"encode from file -> file.gz -> file.gz.base64#gzip ${in_file} && base64 ${in_file.gz} | cat > ${out_file.gz.base64} && rm in_file.gz#"},
                          {"decode base64 & ignore non-ascii characters#base64 -di ${file.base64}#"},
                          {"decode base64 & decode gzip to normal file#base64 -di ${file.gz.base64} | zcat > ${out_file}#"}}},
            ...
            {"sync +", {{"show help#sync --help#"},
                        {"flush all cached file write in fs to disk#sync#"},
                        {"flush certain file write in fs to disk#sync ${file1} ${file2} ...#"},
                        {"flush certain file write in fs to disk (only file content, exclude file metadata)#sync --data ${file1} ${file2} ...#"},
                        {"flush whole fs that contains the given file#sync -f ${file1}#"}}},
