        ...
        {"network +", {
            ...
            {"iptables +", {{"show cmd help#iptables --help#"},
                            ...
                            {"list iptable rules +", {{"*list rules of all chains in table(raw mangle nat filter)#iptables -n -v -L -t ${table}#iptables"},
                                                      ...
                                                      {"search & list rules (item seq num display)#sudo iptables -t nat -nvL --line-numbers | grep '${tcp dpt:61005 to:172.101.0.6:22}'#"}}},
                            {"del certain rule by linno of certain table & chain#sudo iptables -t nat -D PREROUTING ${line_number}#"},
                            {"iptables-save +", {{"show manual page#man iptables-save#"},
                                                 {"show iptable rules in plain txt for certain table#iptables-save -t ${table}#"},
                                                 {"save iptables rules of certain table in file#iptables-save -t ${table} > ${file}#"}}},
                            {"iptables-restore +", {{"restore iptables rules from file (untested)#iptables-restore < ${file}#"}}}}},
            ...
            {"conntrack +", {{"list established connection info: src/dst ip/port#conntrack -L#"},
                             {"del established connection info: src/dst ip/port (clear & reset state before connection related iptable changes)#conntrack -D#"}}},
